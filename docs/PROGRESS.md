...
安装、开发和发版以仓库根目录的 README 为准。本文件只保留过程记录。
...
点击穿透开启时，角色在光标周围约 70px（中心段全透，外圈较硬） 做径向镂空：中心完全透明（看见下层），向外线性渐变到完全不透明。关闭穿透（托盘或设置）后立即去掉镂空；整窗仍忽略鼠标，`setIgnoreMouseEvents(true, { forward: true })` 只用来接收移动。
